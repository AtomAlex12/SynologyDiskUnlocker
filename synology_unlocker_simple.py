...
class SynologyUnlockerSimple:
    def __init__(self, root):
        self.root = root
        self.root.title("Synology Drive Unlocker - Простая версия")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('unlocker_simple.log', encoding='utf-8'),
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            title_frame,
            text="🔓 Synology Drive Unlocker - Простая версия",
            font=('Arial', 16, 'bold'),
            fg='#00ff00',
            bg='#2b2b2b'
        )
        ...
        tk.Entry(settings_frame, textvariable=self.ip_var, width=20, bg='#4b4b4b', fg='#ffffff').grid(row=0, column=1, padx=10, pady=5)
        
        # Логин
        tk.Label(settings_frame, text="Логин:", fg='#ffffff', bg='#3b3b3b').grid(row=1, column=0, sticky='w', padx=10, pady=5)
        tk.Entry(settings_frame, textvariable=self.login_var, width=20, bg='#4b4b4b', fg='#ffffff').grid(row=1, column=1, padx=10, pady=5)
        
        # Пароль
        tk.Label(settings_frame, text="Пароль:", fg='#ffffff', bg='#3b3b3b').grid(row=2, column=0, sticky='w', padx=10, pady=5)
        tk.Entry(settings_frame, textvariable=self.password_var, show='*', width=20, bg='#4b4b4b', fg='#ffffff').grid(row=2, column=1, padx=10, pady=5)
        
        # Использовать sudo
        tk.Checkbutton(
            settings_frame,
            text="Использовать sudo",
    ...
    def save_config(self):
        ...
        try:
            ...
            with open('config_simple.json', 'w', encoding='utf-8') as f:
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists('config_simple.json'):
                with open('config_simple.json', 'r', encoding='utf-8') as f:
                    config = json.load(f)
...
def main():
    root = tk.Tk()
    app = SynologyUnlockerSimple(root)
    root.mainloop()
